...
export default function All() {
  ...
  return data.allFriends.map(f => {
    ...
    return <p key={f.id}>{f.id}, {f.firstName} {f.lastName}, {f.gender}, {f.email} {age}

    &nbsp;<a href="#" onClick={() => onDeleteFullRefetch(f.id)}>Delete (Refetch All)</a>,
  &nbsp;<a href="#" onClick={() => onDeleteUpdateCache(f.id)}>Delete (Update Cache)</a>
